...
Use [GitHub's private security advisory](https://github.com/sera-cx/sera-agents/security/advisories/new) or contact the maintainer (see profile).
...
- x402-service in `live` mode performs real on-chain settlement and requires a vault wallet. Boot refuses without `X402_FACILITATOR_URL` + CDP credentials + `X402_LIVE_ACK=true`; payment verification is wired to the Coinbase CDP facilitator (see [`SECURITY-MODEL.md`](SECURITY-MODEL.md)).
...
- x402-service `live` mode is code-complete against the Coinbase CDP facilitator but **not yet** Base-Sepolia E2E verified; operators must set `X402_LIVE_ACK=true` and complete that test before mainnet (see [`SECURITY-MODEL.md`](SECURITY-MODEL.md)).
